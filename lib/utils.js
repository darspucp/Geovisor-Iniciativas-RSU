...
export const getCifrasPortada = async() => {

  try{
    const dataPortada = [];
    const result = await axios.get(`${API_PORTADA}`); 
      dataPortada.push(
          { numero: result.data.acf.nacionales, descripcion: "Nacionales" },
          { numero: result.data.acf.lima_metropolitana_y_callao, descripcion: "Lima Metropolitana y Callao" },
          { numero: result.data.acf.costa, descripcion: "Costa" },
          { numero: result.data.acf.los_andes, descripcion: "Sierra" },
          { numero: result.data.acf.amazonia, descripcion: "Selva" }
         // { gif:    result.data.acf.mapa_portada, descripcion: "" }
        ) 
...
export const getImageGif = async() => {
  try{
    //const imagen = "";
    const result = await axios.get(`${API_PORTADA}`); 
    //imagen = result.data.acf.mapa_portada
    //let splitGif = result.data.acf.mapa_portada.split("-",2);
  // let gifClean = splitGif[0]+".gif";
      return result.data.acf.mapa_portada;
     //return gifClean;
  }catch (error){
    console.log(error);
  }
